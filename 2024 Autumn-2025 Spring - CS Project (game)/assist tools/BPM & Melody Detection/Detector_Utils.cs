...
public partial class AudioHandler
{
    ...
    double TimePerInstant => (double)InstantSize / SampleRate;
    ...
    List<float> FFT_Abs(List<float> x)
    {
        ...
        // copy the list
        List<float> sample = new(x);

        if ((int)Math.Log2(size) != Math.Log2(size))
        {
            // pad to length = the power of 2
            int len = (1 << ((int)Math.Log2(size) + 1)) - size;
            sample.AddRange(new List<float>(new float[len]));
        }
        // Convert to complex number
        List<NAudio.Dsp.Complex> x_time = [];
        int i = 0;
        foreach (var x_k in sample)
        {
            NAudio.Dsp.Complex c;
    ...
    /// <param name="fileName"></param>
    void SPECTROGRAM<T>(List<List<T>> spectrum, string fileName)
    {
        if (DEBUG)
        {
