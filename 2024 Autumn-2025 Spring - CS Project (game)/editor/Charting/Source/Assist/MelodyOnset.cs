...
public partial class AudioHandler
{
    ...
    List<List<float>> GetSemitoneSpectrum(List<List<float>> spectrum, float min_freq, float max_freq, float freq_gap)
    {
        ...
        foreach (var time in spectrum)
        {
            ...
            var harmonics = HarmonicSum(time, min_freq, freq_gap);
    ...
    public MelodyData GetMainMelodyOnset()
    {
        var amp = GetMono();
        ...
        //SPECTROGRAM(processed_spectrum, "DEBUG_SPECTROGRAM_POST.txt");
        ...
        // to semitone band
        var lead_onset = BandSoftOnsets(melody_band, BASS_MAX_FREQ, MID_MAX_FREQ, freqGap);
        ...
        int chunkID = 0;
        while ((chunkID + 1 + bestGap.Value[1]) * bestGap.Key * timeDiff / TimePerInstant + offsetIndex < lead_onset.Count)
        {
            int lowerChunkID = (int)((chunkID + bestGap.Value[1]) * bestGap.Key * timeDiff / TimePerInstant) + offsetIndex;
            int upperChunkID = (int)((chunkID + 1 + bestGap.Value[1]) * bestGap.Key * timeDiff / TimePerInstant) + offsetIndex;
        ...
        for (int i = 0; i < onset_chunks.Count; i++)
        {
            ...
            for (int j = 0; j < clusters.Count; j++)
            {
                ...
                {
                    for (int k = 0; k < onset_chunks[i].Count; k++)
                    {
                        if (onset_chunks[i][k] > 0 && onsets[j][k] > 0)
                        {
                            onset_post.Add(1);
                        }
                        else
        ...
        return new MelodyData(bpm, offset, lead_onset);
    }
}